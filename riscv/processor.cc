...
processor_t::processor_t(const char* isa, simif_t* sim, cycle_info_t* cycle_info, uint32_t id,
        bool halt_on_reset)
  : debug(false), halt_request(false), sim(sim),  ext(NULL), cycle_info(cycle_info),
  id(id), halt_on_reset(halt_on_reset), last_pc(1), executions(1)
{
